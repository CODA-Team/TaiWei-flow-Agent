...
class OptimizationWorkflow:
    def __init__(self, platform: str, design: str, objective: str):
        ...
        initial_clock_periods = {
            ...
            ('asap7', 'aes'): 500,     # in picoseconds
            ('asap7', 'ibex'): 1000,   # in picoseconds
            ('asap7', 'jpeg'): 500,   # in picoseconds
    ...
    def _calculate_objective(self, run: Dict[str, Any]) -> Dict[str, float]:
        ...
        if self.objective == 'ECP':
            ...
            # tns_eval (<= 0) is appended to logs by eval_tns.sh.
            # Negate to make it positive (smaller = better), then compute
            # improvement ratio against baseline, same convention as ECP/DWL:
            #   ratio < 0 → improved over baseline
            #   ratio > 0 → worse than baseline
            # tns_base: try opt_config.json first, fallback to tns_base.txt
            # (auto-generated by eval_tns.sh from default run in iteration 1).
            tns_base_val = baseline.get('tns_base')
            if tns_base_val is not None:
                tns_base = float(tns_base_val)
            else:
                tns_base_file = os.path.join(
                    'designs', self.platform, self.design, 'tns_base.txt'
                )
                if os.path.exists(tns_base_file):
                    tns_base = float(open(tns_base_file).read().strip())
                    print(f"[TNS_EVAL] Loaded tns_base = {tns_base} from {tns_base_file}")
                else:
                    raise ValueError(
                        f"TNS_EVAL requires tns_base. Not found in opt_config.json "
                        f"or {tns_base_file}. Run iteration 1 first."
                    )
            if tns_base <= 0:
                raise ValueError(f"tns_base must be positive, got {tns_base}")
            if 'tns_eval' in metrics:
                result['value'] = improvement_ratio(-float(metrics['tns_eval']), tns_base)
